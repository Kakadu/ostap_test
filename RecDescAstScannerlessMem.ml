(*
   ...
   Than we compare executing speed we non-memization recursive descent.
*)

open Helpers
open Printf

type r = Parsed of string * lexbuf | Failed
...
module Lexer = struct
  ...
  let is_finished {pos; str_len} = (pos >= str_len)
  ...
  let single_char_parser ch buf =
    if is_finished buf then Failed
      ...
      if buf.str.[buf.pos] = ch then Parsed((),{buf with pos=pos+1})
      else Failed
